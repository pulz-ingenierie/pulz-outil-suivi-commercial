...
1) LE CONTACT (interlocuteur EXTERNE, côté promoteur/structure) : une personne du promoteur/de la structure qui est L'INTERLOCUTEUR / la personne à contacter / le directeur d'agence / le responsable de programme pour une ou plusieurs AFFAIRES (ex. « Rémi Thierry, directeur d'agence, notre contact pour les opérations de Roncq et d'Armentières »). C'est un CONTACT : mets-le dans "contacts" et LISTE les affaires concernées dans son champ "operations", en reprenant le LIBELLÉ EXACT de chaque opération tel qu'il apparaît dans "operations"/"nouvelles_operations" (ex. "operations": ["Nacarat - Roncq - Concours de 200 logements", "Nacarat - La Chapelle-d'Armentières - Construction de 40 logements"]). NE le mets PAS dans "referents".
...
PROGRESSION D'UNE OPÉRATION EXISTANTE (changement de phase) : si le texte indique qu'une affaire DÉJÀ CONNUE avance dans le pipeline (ex. « l'opération X a été chiffrée », « offre déposée », « on passe en négociation », « on a gagné X », « on n'a pas eu X »), propose son NOUVEAU statut dans "changements_phase" : { operation: nom EXACT de l'affaire connue, phase: nouvelle phase }. Repères : « a été chiffrée / chiffrage réalisé » → l'étape « à chiffrer » est FAITE, l'affaire passe à "offre_remise" (offre à déposer/déposée) ; « en négociation / mise au point » → "nego" ; « remportée / gagnée » → "gagne" ; « perdue / non retenue » → "perdu" ; « concours » → "concours". Ne propose un changement QUE si le texte le dit, et UNIQUEMENT pour une opération connue (pour une nouvelle affaire, utilise plutôt nouvelles_operations[].phase).

VILLE D'UNE OPÉRATION EXISTANTE (commune renseignée après coup) : le libellé d'une affaire déjà connue peut contenir « ✕ » à la place de la ville (ex. « Spirit - ✕ - Construction de 80 logements »), car la commune n'était pas connue à sa création. Si le texte PRÉCISE la commune d'une telle affaire connue (ex. « l'opération Spirit des 80 logements, c'est à Poitiers »), propose-la dans "changements_ville" : { operation: nom EXACT de l'affaire connue (tel qu'il apparaît dans "operations", avec son « ✕ »), ville: nom de la commune seul (ex. "Poitiers") }. Cela permettra de compléter le titre (remplacer le « ✕ » par la commune). Ne le propose QUE si le texte donne la commune, et UNIQUEMENT pour une opération connue (pour une nouvelle affaire, renseigne directement "nouvelles_operations[].ville").
