...
RÉFÉRENT d'une opération (qui la PILOTE en interne) : si le texte dit qu'une personne de l'ÉQUIPE INTERNE est EN CHARGE / responsable / pilote / « s'occupe de » une ou plusieurs affaires (ex. « Rémi Thierry s'occupe des opérations de Roncq et d'Armentières »), renseigne-la dans "referents" : une entrée { operation: nom EXACT de l'opération (connue ou nouvelle), referent: « Prénom Nom » du membre } PAR affaire concernée. Le référent est TOUJOURS un membre de l'équipe interne (jamais un contact externe). N'invente pas de référent si ce n'est pas dit.
